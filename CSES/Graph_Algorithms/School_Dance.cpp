...
ll n,m,e;
bool done= false;
void get_path(ll node,ll par,Dinic &dinic) {
    if(node==1)done= false;
    if (pth.size()==2) {
        cout << pth[0] << " " << pth[1] - 501<< endl;
        done = true;
        return;
    }
    for (auto ch: dinic.adj[node]) {
        if (dinic.edges[ch].flow == 1 && par != dinic.edges[ch].u && ch % 2 == 0) {
            pth.push_back(dinic.edges[ch].u);
            dinic.edges[ch].flow = 0;
            get_path(dinic.edges[ch].u, node, dinic);
            pth.pop_back();
            if (done) break;
        }
    }
...
void solve() {
    cin >> n >> m>>e;
    Dinic dinic(1007, 1002,1003);
    G.resize(1007);
    for (int i = 1; i <=n ; ++i) {
        dinic.addEdge(1002, i, 1);
        G[1002].push_back(i);
    }
    for (int i = 1; i <=m ; ++i) {
        dinic.addEdge(i + 501, 1003, 1);
        G[i + 501].push_back(1003);
    }
    for (int i = 0; i < e; ++i) {
        int u, v, w;
        cin >> u >> v;
        dinic.addEdge(u, v + 501, 1);
        G[u].push_back(v + 501);
    }
    ll z=dinic.flow();
    cout <<z << endl;
    while (z--)
    get_path(1002, -1, dinic);
}
signed main() {
