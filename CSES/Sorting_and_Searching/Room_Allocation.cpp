...
void solve( ll hh) {
    ...
    sort(all(a));
    for (int i = 0; i <n ; ++i) {
        ll z = s.order_of_key({a[i].first.first,0});
        if (!z) {
            s.insert({a[i].first.second, f});
            res[a[i].second]=f++;
        } else {
            pi now = *s.find_by_order(z - 1);
            s.erase(now);
            res[a[i].second]=now.second;
            s.insert({a[i].first.second, now.second});
        }
    }
    cout<<f-1<<endl;
